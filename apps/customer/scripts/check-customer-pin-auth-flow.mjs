...
expect('customer platform API exposes PIN endpoints', platformApi.includes('/customer/auth/pin/status') && platformApi.includes('/customer/auth/pin/setup') && platformApi.includes('/customer/auth/pin/verify') && platformApi.includes('/customer/auth/pin/change'))
expect('customer route middleware refreshes expired sessions before PIN redirects', authMiddleware.includes("to.path === '/pin'") && authMiddleware.includes('pinSetupRequired.value || pinRequired.value') && authMiddleware.includes("path: '/pin'") && authMiddleware.includes('refreshAuthToken') && authMiddleware.includes('restoreOrRefreshSession(!userPinStateKnown)') && authMiddleware.includes('restoreOrRefreshSession(true)') && authMiddleware.indexOf('restoreOrRefreshSession(true)') < authMiddleware.indexOf("path: '/pin'"))
...
expect('PIN page supports setup and verify keypad flows', pinPage.includes('platformApi.setupPin') && pinPage.includes('platformApi.verifyPin') && pinPage.includes('setPinVerified(Boolean(response.pin_verified))') && pinPage.includes('pin_confirmation') && pinPage.includes('appendDigit') && pinPage.includes('handleBack') && pinPage.includes('<PinKeypadScreen') && pinPage.includes('clearAuthToken()') && pinPage.includes("path: '/login'"))
...
expect('backend exposes customer PIN auth routes', apiRoutes.includes('/customer/auth/pin/status') && apiRoutes.includes('/customer/auth/pin/setup') && apiRoutes.includes('/customer/auth/pin/verify') && apiRoutes.includes('/customer/auth/pin/change'))
expect('backend controller validates PIN payloads and maps PIN errors', customerAuthController.includes('pinErrors') && customerAuthController.includes('customerPinSetupRequired') && customerAuthController.includes('customerPinRequired') && customerAuthController.includes('customerPinLocked') && customerAuthController.includes("'pin_invalid'"))
expect('backend service hashes PINs and rate-limits failed attempts', customerAuthService.includes('Hash::make($pin)') && customerAuthService.includes('Hash::check($pin') && customerAuthService.includes('PIN_MAX_FAILED_ATTEMPTS') && customerAuthService.includes('PIN_LOCK_SECONDS'))
...
expect('backend customer middleware blocks protected APIs until PIN is unlocked', customerAuthMiddleware.includes('requiresPinUnlock') && customerAuthMiddleware.includes('customerPinSetupRequired') && customerAuthMiddleware.includes('customerPinRequired') && customerAuthMiddleware.includes('customer/auth/pin/setup') && customerAuthMiddleware.includes('customer/auth/pin/verify'))
