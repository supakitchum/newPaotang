...
expect('customer home forces PIN for authenticated sessions while staying public for guests', authMiddleware.includes("const requiresPinBeforeUse = requiresAuth || to.path === '/'") && authMiddleware.includes('requiresPinBeforeUse && !requiresAuth && !isAuthenticated.value && refreshToken.value') && authMiddleware.includes('requiresPinBeforeUse && !isPinPage') && customerAuthRoutes.includes("  '/',"))
...
expect('PIN page supports setup, verify, and OTP reset keypad flows', pinPage.includes('platformApi.setupPin') && pinPage.includes('platformApi.verifyPin') && pinPage.includes('platformApi.requestPinResetOtp') && pinPage.includes('platformApi.verifyPinResetOtp') && pinPage.includes('platformApi.confirmPinResetOtp') && pinPage.includes('setPinVerified(Boolean(response.pin_verified))') && pinPage.includes('pin_confirmation') && pinPage.includes('otp_verification_token') && pinPage.includes('resetStep') && pinPage.includes('ลืม PIN?') && pinPage.includes('appendDigit') && pinPage.includes('handleBack') && pinPage.includes('<PinKeypadScreen') && pinPage.includes('clearAuthToken()') && pinPage.includes("path: '/login'"))
...
expect('backend exposes customer PIN auth routes', apiRoutes.includes('/customer/auth/pin/status') && apiRoutes.includes('/customer/auth/pin/setup') && apiRoutes.includes('/customer/auth/pin/verify') && apiRoutes.includes('/customer/auth/pin/change') && apiRoutes.includes('/customer/auth/pin/reset/verify-password') && apiRoutes.includes('/customer/auth/pin/reset') && apiRoutes.includes('/customer/auth/pin/reset/request-otp') && apiRoutes.includes('/customer/auth/pin/reset/verify-otp') && apiRoutes.includes('/customer/auth/pin/reset/confirm-otp'))
