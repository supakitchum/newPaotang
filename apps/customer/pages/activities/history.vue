<template>
  <MobileShell active-nav="home" show-bottom-nav>
    <BlueHeader title="กิจกรรมงวดย้อนหลัง" back-to="/activities" min-height="214px" />

    <section class="content-sheet flush activities-sheet">
      <div class="activity-draw-filter">
        <div>
          <span>เลือกงวดย้อนหลัง</span>
          <strong>{{ selectedGameLabel }}</strong>
        </div>
        <select
          v-model="selectedGameId"
          class="form-select"
          :disabled="gameOptions.length < 2"
          @change="handleGameChange"
        >
          <option v-if="gameOptions.length === 0" value="">ไม่มีกิจกรรมย้อนหลัง</option>
      ...
      <div v-if="isLoading" class="activities-state">
        ...
        <p>กำลังโหลดกิจกรรมย้อนหลัง</p>
      </div>

      <div v-else-if="activities.length === 0" class="activities-empty">
        <i class="bi bi-clock-history" />
        <h2>ยังไม่มีกิจกรรมย้อนหลัง</h2>
        <p>เมื่อมีรายการกิจกรรมของงวดก่อนหน้า คุณจะเลือกดูย้อนหลังได้จากหน้านี้</p>
        <NuxtLink class="activity-current-link" to="/activities">กลับไปกิจกรรมงวดปัจจุบัน</NuxtLink>
      </div>
      ...
      <div v-else class="activities-list">
        <NuxtLink
          ...
          :key="activity.id"
          class="activity-card"
          :to="{ path: `/activities/${activity.slug}`, query: { from: 'history', game_id: selectedGameId || undefined } }"
        >
...
const gameOptions = computed(() => {
  const games = activityMeta.value?.games

  return Array.isArray(games) ? games : []
})
const selectedGameLabel = computed(() => {
  const selected = gameOptions.value.find((game: Record<string, any>) => String(game.id) === String(selectedGameId.value))

  return selected ? gameLabel(selected) : 'กิจกรรมงวดย้อนหลัง'
})

const gameLabel = (game: Record<string, any>) => {
  const label = String(game.label || game.name || game.code || game.id || '').trim()

  return label || 'งวดกิจกรรม'
}
...
const loadActivities = async () => {
  ...
  try {
    ...
    const gameId = String(route.query.game_id || '').trim()
    const params = gameId
...
const handleGameChange = async () => {
  const nextId = String(selectedGameId.value || '')
  const query = nextId ? { game_id: nextId } : {}

  await navigateTo({ path: '/activities/history', query }, { replace: true })
}
...
useTenantSeo({
  title: 'กิจกรรมงวดย้อนหลัง',
  description: 'ดูกิจกรรมของร้านค้าในงวดก่อนหน้า',
  canonicalPath: '/activities/history'
})
</script>
...
.activity-draw-filter div {
  display: grid;
  gap: 3px;
  min-width: 0;
}

.activity-draw-filter span {
  color: #94a3b8;
  font-size: 12px;
  font-weight: 900;
}

.activity-draw-filter strong {
  color: #1f2f54;
  font-size: 15px;
  font-weight: 900;
  overflow: hidden;
...
.activity-draw-filter .form-select {
  border-color: #dbe6f3;
  border-radius: 999px;
  color: #1f2f54;
  font-size: 13px;
