type LotteryResultRealtimeStatus = 'idle' | 'unavailable' | 'connecting' | 'connected' | 'reconnecting' | 'error'

type RealtimeValue<T> = T | { value: T }

type LotteryResultRealtimeOptions = {
...
export const useLotteryResultRealtime = (options: LotteryResultRealtimeOptions = {}) => {
  const config = useRuntimeConfig()
  const status = ref<LotteryResultRealtimeStatus>('idle')
  const error = ref('')
  ...
  function connect(reconnecting = false) {
    ...
    activeSocket.addEventListener('error', () => {
      if (activeSocket === socket) {
        status.value = 'error'
        error.value = 'Lottery realtime connection failed.'
      }
    })
  ...
  function disconnect(finalStatus: LotteryResultRealtimeStatus = 'idle') {
    stopReconnectTimer()
    cleanupSocket(true)
    socketId = ''
    currentSocketUrl = ''
...
const buildRealtimeSocketUrl = (baseUrl: string, key: string) => {
  ...
  return `${url}${separator}protocol=7&client=newpaotang-lottery-result&version=1.0&flash=false`
}
