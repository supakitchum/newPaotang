...
for (const evidence of [
  ...
  ['partner BO local .test host allowed', nuxtConfig.includes("allowedHosts: ['.test']")],
  ...
  ['partner BO login submits tenant scope without tenant id', loginPage.includes("const scope = isPartnerBoMode.value ? 'tenant'") && loginPage.includes('tenant_id: isPartnerBoMode.value ? null') && apiClient.includes('const body = partnerMode') && apiClient.includes("scope: 'tenant'") && apiClient.includes('const tenantId = partnerMode ? null')],
  ...
  ['central BO login fields remain', loginPage.includes('<label class="form-label">Scope</label>') && loginPage.includes('<option value="central">Central</option>') && loginPage.includes('<label class="form-label">Tenant ID</label>')],
...
for (const evidence of [
  ...
  ['stock manager consolidated menu workflow', operationsCatalog.includes("'central:stock-generation': { target: 'stock', title: 'Stock Manager' }") && operationsCatalog.includes("'central:master-stock': { target: 'stock', title: 'Stock Manager' }") && adminNavigation.includes("new Set(['master_stock', 'partner_quotas', 'stock_recall'])") && adminNavigation.includes("item.key === 'stock_generation' ? 'Stock Manager' : item.label") && !adminNavigation.includes("'central:master_stock': '/admin/central/master-stock'") && !adminNavigation.includes("'central:stock_recall': '/admin/central/stock-recall'")],
