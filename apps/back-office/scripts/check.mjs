...
for (const evidence of [
  ...
  ['sidebar horizontal Siamblend logo', adminFoundationCss.includes('.np-sidebar-brand-header') && adminFoundationCss.includes('width: 15rem') && adminFoundationCss.includes('.np-admin-brand-logo.desktop-logo') && adminFoundationCss.includes('display: block !important')],
  ['sidebar brand stays above the fixed app header', adminFoundationCss.includes('.app-sidebar {\n  z-index: 1040 !important;') && adminFoundationCss.includes('.np-sidebar-brand-header') && adminFoundationCss.includes('z-index: 1041 !important;') && adminFoundationCss.includes('.app-header {\n  z-index: 1030;')],
  ['mobile sidebar vertical-only scrolling', adminFoundationCss.includes('.app-sidebar .simplebar-content-wrapper') && adminFoundationCss.includes('touch-action: pan-y') && adminFoundationCss.includes('.simplebar-track.simplebar-horizontal') && adminFoundationCss.includes('overflow-x: hidden !important')],
  ['independent sidebar and content scrolling', adminLayout.includes('np-admin-content-scroll') && adminFoundationCss.includes('.np-admin-content-scroll') && adminFoundationCss.includes('height: calc(100dvh - 4.25rem)') && adminFoundationCss.includes('overscroll-behavior: contain') && adminFoundationCss.includes('.np-admin-content-scroll .footer')],
...
for (const evidence of [
  ...
  ['tenant order totals preserve money minor-unit formatting', operationsCatalog.split("{ key: 'total', label: 'Total', type: 'money' }").length >= 3 && !operationsCatalog.includes("{ key: 'total.amount', label: 'Total', type: 'money' }")],
...
for (const evidence of [
  ...
  ['header horizontal brand marker', adminHeader.includes('data-admin-header-logo="horizontal"') && adminHeader.includes('np-header-brand-link')],
  ['header server clock sync', adminHeader.includes('data-admin-server-clock') && adminHeader.includes("'/auth/admin/server-time'") && adminHeader.includes("document.addEventListener('visibilitychange'") && adminHeader.includes('5 * 60 * 1000')],
  ...
  ['sidebar horizontal Siamblend brand', adminSidebar.includes('centralLogoUrl') && adminSidebar.includes('data-admin-sidebar-logo="horizontal"') && adminSidebar.includes('data-admin-sidebar-logo="horizontal-dark"') && adminSidebar.includes('np-sidebar-brand-header')],
