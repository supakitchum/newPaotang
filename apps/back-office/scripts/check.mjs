...
for (const evidence of [
  ...
  ['central stock table realtime workflow', operationsPage.includes('private-admin.central.stock.table.game') && operationsPage.includes("eventName: 'stock.table.updated'") && operationsPage.includes('showStockTableRealtimePanel') && operationsPage.includes('Stock table realtime') && operationsPage.includes('stockTableRealtimeEnabled') && operationsPage.includes('stockTableRealtimeStatusLabel') && operationsPage.includes('Select a game to show stock summary widgets and enable live table updates.') && operationsPage.includes('handleStockTableRealtimeEvent') && operationsPage.includes('handleStockTableRealtimeReconnect') && operationsPage.includes('reloadStockTableFromRealtime') && operationsPage.includes('stockTableRealtimeRowRequiresReload') && operationsPage.includes('mergeStockTableRealtimeRow') && operationsPage.includes('refreshStockSummaryWidgets') && operationsPage.includes('useAdminRealtimeSubscription')],
  ['stock manager consolidated menu workflow', operationsCatalog.includes("'central:stock-generation': { target: 'stock', title: 'Stock Manager' }") && operationsCatalog.includes("'central:master-stock': { target: 'stock', title: 'Stock Manager' }") && adminNavigation.includes("new Set(['master_stock', 'partner_quotas'])") && adminNavigation.includes("item.key === 'stock_generation' ? 'Stock Manager' : item.label") && !adminNavigation.includes("'central:master_stock': '/admin/central/master-stock'")],
