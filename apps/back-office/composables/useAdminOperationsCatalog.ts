...
const tenantThemeFields: OperationFormField[] = [
  ...
  { key: 'theme.text_color', label: 'Text color', type: 'color', sourceKey: 'theme.text_color', defaultValue: '#242833' },
  { key: 'theme.font_family', label: 'Font family', sourceKey: 'theme.font_family', defaultValue: 'Kanit' },
]
...
const billingPlanDetailFields: OperationColumn[] = [
  ...
  { key: 'code', label: 'Code' },
  { key: 'name', label: 'Name' },
  { key: 'monthly_fee', label: 'Monthly fee', type: 'money' },
...
const tenant: OperationResource[] = [
  ...
  },
  {
    ...
    actions: [
      { key: 'approve', label: 'Approve', endpoint: '/admin/tenant/reward-claims/{claim_id}/approve', variant: 'success', reason: true, optionalReason: true, enabledStatuses: ['submitted', 'under_review'], hideWhenDisabled: true, contextFields: rewardClaimActionContext },
    ...
    listSections: [
      {
        key: 'pending-reward-claims',
        title: 'Pending Exchange Requests',
        listEndpoint: '/admin/tenant/reward-claims',
        detailEndpoint: '/admin/tenant/reward-claims/{claim_id}',
        idParam: 'claim_id',
        columns: [
        ...
        defaultSort: { key: 'submitted_at', direction: 'asc' },
        defaultQuery: { section: 'pending' },
        emptyTitle: 'No pending exchange requests',
        emptyMessage: 'Reward exchange requests waiting for approval will appear here.',
      },
      {
        key: 'reward-claim-history',
        title: 'Exchange Reward History',
        listEndpoint: '/admin/tenant/reward-claims',
        detailEndpoint: '/admin/tenant/reward-claims/{claim_id}',
        idParam: 'claim_id',
        columns: [
          ...
          { key: 'payout_method', label: 'Payout method' },
          { key: 'status', label: 'Status', type: 'status' },
          { key: 'updated_at', label: 'Last action', type: 'datetime', fallbackKeys: ['reviewed_at', 'paid_at', 'created_at'] },
        ],
        filters: cursorFilters([
          statusFilter(['approved', 'rejected', 'cancelled', 'paid']),
          { key: 'q', label: 'Search' },
          { key: 'game_id', label: 'Game ID' },
          { key: 'customer_id', label: 'Customer ID' },
        ]),
        ...
        defaultSort: { key: 'updated_at', direction: 'desc' },
        defaultQuery: { section: 'history' },
        emptyTitle: 'No exchange reward history',
        emptyMessage: 'Approved, rejected, cancelled, and paid reward exchanges will appear here.',
      },
    ],
  ...
  },
  {
    ...
    settingsFields: [
      ...
      { key: 'config.checkout.external_payment.provider', label: 'External checkout provider', placeholder: 'Runtime provider key', help: 'Required when external checkout is enabled. This value is also used by the payment webhook route.' },
      { key: 'config.checkout.external_payment.redirect_url_template', label: 'External checkout redirect URL template', placeholder: 'https://provider.example/checkout?order={order_id}&reference={reference}', help: 'HTTPS only. Supported placeholders: {order_id}, {reference}, {amount}, {amount_minor}, {currency}, {tenant_id}, {customer_id}, {callback_url}.' },
