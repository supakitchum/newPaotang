...
| home reward summary / result index | `GET /reward` | `GET /public/games/current`, `GET /public/results/live/{game_id}`, `GET /public/results/{game_id}`, `GET /public/results/latest`, `GET /public/results/history?limit=3&exclude_game_id={game_id}` | Home and modern `/result` remain current-game aware: load the current game, prefer its live result, fall back to its published result, and keep the current game as a pending result only when both result calls return 404. The result index loads up to three latest published draws, excluding the featured game; older deployments without the history route fall back to the previous latest-result behavior. While the current game is actively selling, Home displays the first resolved result from the selected/history bundle, so the latest published draw remains visible instead of the unresolved current-draw placeholder. Legacy `/results` retains its separate live-latest source but uses the same published history list. Flutter result parsing accepts recursive `data`/`resource`/`result`/`rewardResult`/summary wrappers, camelCase fields, nested game context, object scalar IDs/status/dates/completion values, `rewardItems`/`prizeItems`, array number aliases, and hyphenated prize types. Maintenance/auth/PIN/suspension errors are rethrown into the shared operational route flow; ordinary history failures degrade to the latest published result without hiding the featured result. |
...
| customer notifications | Flutter Home bell, `/notifications`, native FCM lifecycle | `GET /customer/notifications`, `GET /customer/notifications/unread-count`, `PATCH /customer/notifications/{notification_id}/read`, `POST /customer/notifications/read-all`, `POST /customer/notification-devices`, `GET /customer/notification-devices/{installation_id}/status`, `DELETE /customer/notification-devices/{installation_id}` | Server recipient rows own unread state across Web/iOS/Android, including read-all availability when unread rows are beyond the loaded cursor page. Flutter cursor-loads the inbox, invalidates count/list after private `customer.notification.created` and `.read` events, marks one item read before opening an allowlisted route, and retains push destinations through the existing Login/PIN gate. Native registration starts only after authenticated PIN unlock, refreshes the FCM token, bounds and allowlists runtime app version/build plus non-identifying model/OS diagnostics to the exact API contract, and reconciles the current installation with the server at session start and at most every 15 minutes while resuming. A missing row is registered, while a server-revoked row causes the rejected local token to be deleted and replaced before registration. Backend rejects registration when a revoked installation submits the same token hash, so an invalid token cannot be reactivated even by an older or racing client. Unchanged active registrations refresh daily, failures retry with bounded backoff and safe stage/error-type diagnostics, and explicit logout revokes the installation and deletes the local token. Unknown metadata fields such as hardware identifiers and oversized values are rejected server-side. Optional foreground local-notification setup/display failures do not disable remote FCM registration/background delivery or the authoritative foreground inbox refresh. Web keeps inbox/realtime without browser push. Device tokens and hashes never appear in status resources or logs; terminal FCM revocations retain only a safe reason and timestamp. Tenant history reports aggregate sent/pending/failed counts plus a partial state when devices differ. |
...
| LINE login and account linking | LINE OAuth browser callback | Web/PWA: existing LINE OAuth launch/callback; iOS/Android: `POST /customer/auth/line/native` | Native Flutter initializes the official LINE SDK from the tenant runtime `mobile.line.native_channel_id`, requests `profile` and `openid`, and sends only the returned access token, purpose, and safe redirect to Platform API. Platform verifies the token against LINE, requires its verified `client_id` to match the tenant's configured LINE Login channel, fetches the profile/friendship state server-side, and then reuses the existing identity login, first-time OTP/member onboarding, password-reset, account-linking, centralized PIN, and one-active-device handoff. Authenticated linking still requires a PIN-unlocked session. Channel secrets and provider tokens are never exposed through bootstrap or stored as the customer platform session. If native configuration is absent, iOS/Android fall back to the existing browser OAuth flow; Web/PWA always retains that flow. |
...
- Native LINE bootstrap note: Platform mobile bootstrap exposes only the
  tenant's public LINE Login Channel ID and native-enabled state as
  `mobile.line.native_channel_id` and `native_login_enabled`; the encrypted
  Channel Secret remains server-only. iOS and Android consume these fields for
  the official LINE SDK, while Web/PWA continues to use the browser callback.
...
CustomerNotification.image_url/image_thumb_url -> fixed-ratio inbox campaign media with server-authoritative fallback
```

## Public Relations Campaigns

- The former Tenant BO customer-notification composer is now the tenant-scoped
...
- Affiliate lifecycle notifications use the same authoritative inbox,
  realtime unread badge, and native FCM delivery pipeline. Registration and
  store-name review route to `/affiliate`; tier campaign start, ending-soon,
  final unchanged result, and tier change route to `/affiliate/rankings`;
  commission approval/reversal routes to `/affiliate/commissions`; payout
  submission/status changes route to `/affiliate/withdraw`. Account
  activation/restriction also returns to the Affiliate overview. Every event
  has a transition-specific dedupe key so retries and repeated campaign
  finalization do not create duplicate customer notifications.
