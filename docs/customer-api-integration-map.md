...
| search lotteries | `POST /lotteries/search` | `GET /public/stock/search?number=...&random_seed=...` | Adapter converts `n1..n6/full_number` to `number`, keeps Nuxt-style seeded partial/random search ordering, preserves Nuxt's legacy `/search` alias for `/buy/search`, keeps `/buy/more` as an unseeded same-number lookup without manual refresh controls, maps buy availability such as `bet_status`/`can_buy` into UI disabled state, and preserves API payload messages on stock load failures while hiding internal/client exception text |
