...
| login | `POST /login` | `POST /customer/auth/login`, `POST /customer/auth/login/otp/resend`, `POST /customer/auth/login/otp/verify` | When the tenant has an active SMS OTP provider, a valid phone/password login returns `202` with a short-lived `login_challenge_token`, masked phone, and resend interval instead of issuing a customer session. Flutter and Nuxt keep the customer on Login, verify the six-digit OTP through the challenge endpoint, and only then save the returned access/refresh tokens and continue to the existing PIN gate or safe redirect. The challenge is tenant-scoped and one-time; invalid credentials never send OTP, and no `last_login_at`, wallet, or auth-session mutation occurs before OTP succeeds. Tenants without an active SMS provider retain direct login compatibility. Session restore/refresh and returning-app PIN or biometric unlock do not repeat login OTP because they are not new credential logins. Flutter accepts direct sessions plus recursive `data`/`result`/`resource` envelopes, camelCase token/PIN/customer aliases, and preserves API error copy without exposing internal exceptions. |
