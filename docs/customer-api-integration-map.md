...
| register | `POST /register` | `POST /customer/auth/register` | Adapter preserves current register page, maps token/user, accepts recursive session wrappers and camelCase token/PIN/customer aliases, requires a non-empty OTP verification token before submitting OTP-backed registration, returns to safe redirect or `/pin?redirect=...` when PIN is required, and preserves API payload error messages after OTP. Native/Web autofill hints and the Nuxt-exact OTP panel are presentation metadata only: the step still requests `/customer/auth/otp/request` with purpose `register`, verifies through `/customer/auth/otp/verify`, and sends the returned token in the existing register payload without an alternate client-side POST path. |
