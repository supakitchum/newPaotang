...
7. ai-sub-agents/workflow/execution-mode.md
8. ai-sub-agents/workflow/background-runner.md เมื่ออยู่ AUTO Mode
9. ai-sub-agents/workflow/dependency-graph.md
10. ai-sub-agents/workflow/worktree-start-gate.md
11. ai-sub-agents/workflow/shared-file-locks.md เมื่อ task มี shared file risk
12. ai-sub-agents/workflow/qa-browser-env.md เมื่อ task มี browser QA
13. ai-sub-agents/workflow/memory-maintenance.md
14. role file ของตัวเองใน ai-sub-agents/roles
15. memory ของตัวเองใน ai-sub-agents/memory/<agent>/memory.md
16. trigger file ที่เปิดงานให้ตัวเอง
17. task/prompt ที่ได้รับจาก Orchestrator
18. docs/source-of-truth ที่ task อ้างถึง
```
...
1. Coordinator รับงานจาก chat แล้วเขียน scope, milestone, acceptance criteria, และ agent assignment ลง Markdown
2. Coordinator สร้าง trigger ให้ Orchestrator เท่านั้น
3. Orchestrator แตกงานเป็น task prompt และ trigger ให้ sub-agent ที่เกี่ยวข้อง
4. ทุก agent ต้องผ่าน worktree start gate ก่อนเริ่มงาน
5. Dev agents ทำงานเฉพาะ ownership ของตัวเอง และต้องทำ automated test ประกอบ feature
6. ถ้าแก้ shared file ต้องมี lock file และ release evidence
7. ทุก dev-agent ส่ง handoff กลับ Orchestrator
8. Orchestrator ตรวจว่า handoff ครบ, trigger DONE, lock RELEASED, และงานพร้อม QA
9. QA Tester ทดสอบทุกอย่างบน test env/test DB ก่อนเท่านั้น
10. QA Tester เปิด Google Chrome จริงแบบ visible สำหรับ browser acceptance test และบันทึก evidence
11. Coordinator อ่าน QA report แล้วตัดสิน:
   - `PASS`: ส่ง GitOps
   - `FAIL` หรือ `PASS WITH RISK`: ส่งกลับ Orchestrator เพื่อแตก remediation task
12. Coordinator สร้าง trigger ให้ GitOps หลัง approve เท่านั้น
13. GitOps ทำงานหลัง Coordinator approve และ trigger ถูกต้องเท่านั้น

## Working Areas

```text
ai-sub-agents/tasks       task prompt จาก Orchestrator
...
ai-sub-agents/runner      AUTO Mode claim, heartbeat, and runner logs
```
...
QA browser acceptance ต้องเปิด Google Chrome จริงแบบ visible
...
visible Chrome QA ต้องประกาศ browser URL, API base, test DB, account/role, และ evidence path
