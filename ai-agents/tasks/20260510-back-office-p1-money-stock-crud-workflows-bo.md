...
Coordinator restarted the P1 BO implementation from the cleaned `develop` branch:
...
This is not a new scope. It is the clean restart of the same P1 BO implementation priority after old Orchestrator dispatch files were cleared.
...
If the worktree is dirty, stop and report before editing. Do not recreate `codex/*` branches unless the user or Coordinator explicitly asks for per-agent branches again.

This task file may advance `develop` with orchestration-only files. BO implementation must still treat `f44bee4592e9e012f406229f9f8fbbb2c61c8571` as the clean application baseline and verify that no backend/customer/OpenAPI implementation changes are included in the dispatch commit.

## Objective

Implement the first Back Office P1 money/stock CRUD/API workflows against the frozen backend contract.

Move the P1 rows from generic/partial operator coverage toward real BO workflows. Use typed domain forms/modals where practical, wire through the existing BO API layer, preserve tenant/security behavior, update the CRUD coverage matrix, then hand off to QA for real menu workflow testing.
...
```text
build typed domain forms/modals where practical
...
1. Sync to `develop` and verify baseline/status using the Branch And Baseline Rules above.
2. Read the Source Of Truth files.
3. Confirm Docker runtime policy and Code Agent Commit Rule.
...
If seeded backend data or backend route evidence is needed, use Docker only:
