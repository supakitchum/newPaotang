...
const artifactDir = process.env.QA_ARTIFACT_DIR || '/workspace/ai-agents/reports/artifacts/20260512-back-office-p5-master-stock-commission-transactions-list-action-remediation-qa/browser'
...
const visibleText = async (text) => await page.getByText(text, { exact: false }).count() > 0
...
await stockFilterCard.locator('input').nth(2).fill('20')
...
summary.uiChecks.stockFilterLabels = (await stockFilterCard.locator('label').allTextContents()).map((entry) => entry.trim()).filter(Boolean)
summary.workflows.stock_number_filter_removed = !summary.uiChecks.stockFilterLabels.some((entry) => entry.toLowerCase() === 'number')
summary.workflows.stock_rows_expose_inspection_context = await visibleText('910001')
  && await visibleText('910')
  && await visibleText('001')
  && await visibleText('01')
summary.observations.stockSupportedFilterRowCount = await rowCount()
summary.workflows.stock_supported_filter_has_rows = summary.observations.stockSupportedFilterRowCount > 0
...
summary.workflows.stock_export_number_field_removed = summary.observations.stockExportModal.ticketNumberFieldVisible === false
...
summary.workflows.commission_rows_expose_inspection_context = await visibleText(fixture.affiliate_account_id)
  && await visibleText(fixture.order_id)
  && await visibleText(fixture.commission_rule_id)
  && await visibleText('commission')
  && await visibleText('Calculated')
...
summary.observations.commissionApproveModal = {
  ...
  includesOrderId: (await modalText()).includes(fixture.order_id),
  includesCommissionRuleId: (await modalText()).includes(fixture.commission_rule_id),
  includesTransactionType: (await modalText()).includes('commission'),
  includesCalculatedStatus: (await modalText()).includes('calculated') || (await modalText()).includes('Calculated'),
  includesAmount: (await modalText()).includes('1500') || (await modalText()).includes('15.00'),
}
summary.workflows.commission_approve_context_complete = Object.entries(summary.observations.commissionApproveModal)
...
summary.networkChecks = {
  ...
  stockListCallsOnlySupportedQueryParams: network
    .filter((entry) => entry.method === 'GET' && entry.path.startsWith('/admin/central/stock'))
    .every((entry) => {
      const query = new URLSearchParams(entry.path.split('?')[1] || '')
      return Array.from(query.keys()).every((key) => ['game_id', 'status', 'cursor', 'limit'].includes(key))
    }),
  stockExportPayloadOnlySupportedFields: network
    .filter((entry) => entry.method === 'POST' && entry.path === '/admin/central/stock/exports')
    .every((entry) => entry.payload_keys.every((key) => ['game_id', 'filters', 'filters.status', 'reason'].includes(key))),
