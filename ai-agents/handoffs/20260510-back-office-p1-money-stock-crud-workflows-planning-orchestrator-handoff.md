...
Created a fresh BO Develop task after the branch cleanup/restart:
...
Coordinator restarted orchestration on:

```text
branch: develop
Coordinator clean application baseline: f44bee4592e9e012f406229f9f8fbbb2c61c8571
```

The task instructs BO Develop to sync/verify `develop`, report dirty state before editing, and avoid recreating `codex/*` branches unless explicitly requested.
...
Local static reads such as `git status --short`, `git status --short --branch`, `git rev-parse HEAD`, `rg`, `sed`, `ls`, and `git diff --check` are allowed.
